# -*- coding: utf-8 -*-
"""استخراج تعریف UI از manga_app.py — بدون اجرای گرادیو.

فایل manga_app.py را با AST می‌خواند و همه کامپوننت‌های gr.* (Dropdown,
Radio, Checkbox, Textbox, Slider, File, HTML …) را به ترتیب سورس به
manifest (JSON) تبدیل می‌کند. اپ اندروید فرم بومی را از همین manifest
می‌سازد — یعنی هر دکمه/گزینه‌ای که در وب اضافه شود، اپ هم همان را دارد.

خروجی: dict با sections/fields. مقادیر پیش‌فرض داینامیک (cfg.get(...))
به‌صورت {"cfg": کلید, "default": پیش‌فرض} برگردانده می‌شوند.

حلقه‌های ساخت ویجت (مثل حلقهٔ آپلود فونت‌های لحن روی FONT_BUNDLES) هم
unroll می‌شوند: مقادیر لیترال ماژول (FONT_BUNDLES، SLOT_LABELS …) خوانده
و برای هر آیتم یک ویجت مستقل با id یکتا (<base>_<slot>) ساخته می‌شود —
پس هر اسلاتی که به manga_app.py اضافه شود خودکار در اپ هم می‌آید.
"""
import ast
import copy
import json
...
_UNKNOWN = object()  # مقداری که از AST قابل ارزیابی نیست


class _Env:
    """محیط مقادیر شناخته‌شده (لیترال‌های ماژول + متغیرهای حلقه)."""

    def __init__(self, tree):
        self.scopes = [{}]
        for node in tree.body:  # انتساب‌های سطح بالای ماژول
...
def _eval(node, env):
    """ارزیابی محدود عبارات AST — فقط الگوهای امن و ساده."""
    if node is None:
        return _UNKNOWN
    ...
    if isinstance(node, ast.IfExp):
        ...
        if c is _UNKNOWN:
            # هر دو شاخه یکی‌اند؟ (رایج: {' ✓' if have else ''})
    ...
    if isinstance(node, ast.Call):
        f = node.func
        # len(x) / list(x) / str(x) / dict.get(k, d)
...
def _eval_iterable(expr, env):
    """مقادیر iterable یک عبارت — اگر قابل ارزیابی بود، وگرنه None."""
...
def _iter_values(node, env):
    """مقادیر iterable یک For — اگر قابل ارزیابی بود، وگرنه None."""
...
def _bind(target, value):
    """تخصیص الگو: Name یا Tuple از Name‌ها → dict؛ نشد None."""
    binding = {}
...
class _Snap:
    """اسنپ‌شات محیط در لحظهٔ ثبت Call — برای ارزیابی لیبل f-string داخل حلقه."""
...
def _str_of(node, env=None):
    """مقدار رشته‌ای ثابت؛ f-string با ارزیابی best-effort بخش‌های ممکن."""
    if node is None:
        return None
    if env is not None and isinstance(node, (ast.JoinedStr, ast.FormattedValue)):
        ...
        for x in node.values:
            ...
            if isinstance(v, str):
                ...
                continue  # بخش ارزیابی‌نشدنی (مثل {' ✓' if have else ''}) → رد
            elif v is not None:
                parts.append(str(v))
        s = "".join(parts)
...
def _default(node):
    """value=literal یا value=cfg.get("k", default) یا str(cfg.get(...)) → marker."""
    if node is None:
        return None, None
    if isinstance(node, ast.Call):
        f = node.func
        # str(cfg.get(...)) و امثالش را باز کن
...
def _unwrap_safe(tree):
    """_safe(gr.Slider, 1, 8, value=…) → گره ساختگی gr.Slider(1, 8, value=…).

    اسلایدرهای پیشرفته وب داخل _safe() پیچیده شده‌اند؛ چون gr.X فقط
    ارجاع کلاس است، یک Call جدید با func=gr.X و آرگومان‌های بیرونی ساخته می‌شود.
    """
...
def extract(path_or_source, is_source=False):
    ...
    env = _Env(tree)
    calls = []  # (lineno, node, parent, tag)

    class V(ast.NodeVisitor):
        def __init__(self):
            self.stack = []
            self.tag = ""  # پسوند حلقه (مثلاً نام اسلات لحن)
            self.seq = 0

        def generic_visit(self, node):
            # انتساب‌های قابل ارزیابی (slots، half، SLOT_LABELS محلی …) را
            # در محیط نگه دار — به ترتیب سورس
            if isinstance(node, ast.Assign) and len(node.targets) == 1 \
                    and isinstance(node.targets[0], ast.Name):
                v = _eval(node.value, env)
                if v is not _UNKNOWN:
                    env.scopes[0][node.targets[0].id] = v
            # unroll حلقه‌های ساخت ویجت روی لیست‌های قابل ارزیابی
    ...
    # بخش‌بندی: steptitle → بخش؛ Accordion (فونت‌ها، پیشرفته، لاگ …) هم بخش
    # مستقل می‌شود (مثل وب که آکاردئون است) — فیلدهای مخفی حذف می‌شوند.
    ...
    for it in items:
        if it["type"] == "section":
            ...
            if not (it.get("label") or it.get("info") or it.get("placeholder")):
                continue
            # فیلدهای داخل حلقه (آپلود لحن‌ها) همه یک id می‌گیرند → یکتا کن
            fid = it.get("id")
            if fid:
    ...
    # بخش‌های بی‌فیلد (مثل لاگ زنده که در اپ بومی جای خودش را دارد) حذف
