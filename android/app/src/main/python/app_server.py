# -*- coding: utf-8 -*-
"""سرور وب داخلی اپ اندروید — طراحی مثل وب gradio (همان CSS).

فقط کتابخانه استاندارد پایتون + manga.py. صفحه اصلی همان فرم مرحله‌ای وب
(ورودی → هوش مصنوعی → خروجی)، لاگ زنده، نمایشگر خواندن و دیباگ.
"""
import json
import os
import re
...
# --- خواندن مستقیم از manga_app.py آپدیت‌شده (هر تغییری در توکن‌های
# طراحی/برچسب‌ها/نسخه، خودکار وارد اپ می‌شود). اگر شکست، مقادیر پیش‌فرض.
try:
    from manga_app import (  # noqa: F401
        APP_NAME, APP_VER, PROVIDERS, WEB_CSS,
    )
except Exception:
    APP_NAME, APP_VER, PROVIDERS = "مانگا مترجم", "1.5", [
